...
async def start_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"/memory — Was ich über dich weiß (Profil)\n"
        ...
        f"📝 *ToDos & Tagesrhythmus*\n"
        f"/todo — Heute fällige ToDos (priorisiert)\n"
        f"/todo add <text> [datum] — Neuen ToDo anlegen (Datum: heute|morgen|freitag|30.05)\n"
        f"/todo list [today|week|all] — Liste in gewünschtem Zeitraum\n"
        f"/todo done <id> — ToDo als erledigt markieren\n"
        f"/todo snooze <id> <tage> — Vertagen\n"
        f"/todo drop <id> — Verwerfen ohne Erledigung\n"
        f"/todo show <id> — Details + Meeting-Kontext\n"
        f"/todo stats — Übersicht (offen/vertagt/done/dropped)\n"
        f"/briefing — 🌅 Morgen-Briefing manuell (sonst auto 07:30)\n"
        f"/reflect — 🌙 Tagesabschluss manuell (sonst auto 21:30)\n"
...
def _cal_label(cal_num: int) -> str:
    return "Kalender 1 (Familienkalender)" if cal_num == 1 else "Kalender 2 (Bennoschwede@gmail.com)"
...
async def kalender1_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ Aktiver Kalender: *Kalender 1 (Familienkalender)*", parse_mode="Markdown")
